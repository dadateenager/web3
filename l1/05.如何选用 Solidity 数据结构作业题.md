...
asnwer:使用整数存储的时候，如果是不允许负数那就用Uint，如果允许负数就要用int。
...
asnwer：可变长的用string，不需要字符编码的用bytes。string不能直接比较，需要用keccak256。
...
answer：mapping用于创建KV映射，常用于存储关联数据，在数据查找方面更高效。
...
answer：mapping适合用于账户余额等快速查找和更新键值对的场景，而array适合用于元素顺序重要或者需要迭代处理的场景。
...
answer：利用struct来定义列，然后mapping来存储struct实例，模拟行的概念。
...
answer：`enum` 限制变量的取值范围，减少非法值的输入，提高代码的可维护性和错误预防。
...
answer：因为bytes不涉及UTF-8编码处理，所以在处理不需要字符处理功能的纯二进制数据时候，bytes更节省空间和gas成本
...
answer：直接使用uint256，因为他可以和EVM的时间函数兼容。
...
answer：当数据项逻辑上属于同一实体或需要一起处理时，应将它们封装在一个 `struct` 内部以增加可读性和可维护性。
...
answer：mapping本身不支持迭代，但是可以利用数组来存储mapping的K，然后利用K来进行迭代访问mapping。
...
answer：可以使用 `mapping` 将资产类型（如 ERC20 代币地址）映射到另一个 `mapping`，后者将用户地址映射到余额。  或者是[用户类型]  [资产类型]到余额，然后利用struct来管理多个资产。
...
answer：状态转换逻辑需要包含状态验证逻辑，确保合约按照预定流程转换。
